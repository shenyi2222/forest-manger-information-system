...
namespace AppBoxPro.Dapper.admin
{
    public partial class user : PageBase
    {
        ...
        private void BindGrid()
        {
            ...
            {
                builder.AddWhere("users.Enabled = '" + (rblEnableStatus.SelectedValue == "enabled" ? 1 : 0) + "'");
            }
        ...
        private void SetSelectedUsersEnableStatus(bool enabled)
        {
            ...
            // 执行数据库操作
            DB.Execute("update users set Enabled = '" + (enabled ? 1 : 0) + "' where ID in (" + IDs + ")");  //对bool数值添加引号，List数组改为拼接字符串
