...
namespace AppBoxPro.Dapper.admin
{
    public partial class role_user_addnew : PageBase
    {
        ...
        private void BindGrid()
        {
            ...
            // 排除已经属于本角色的用户
            builder.AddWhere("users.ID not in (select ID from users inner join roleusers on users.ID = roleusers.UserID where roleusers.RoleID = @RoleID)");
        ...
        protected void btnSaveClose_Click(object sender, EventArgs e)
        {
            ...
            DB.Execute("insert into roleusers (UserID, RoleID) values (@UserID, @RoleID)", newids.Select(u => new { UserID = u, RoleID = roleID }).ToList());
